    ...
    try:
        ...
        def iter_from_X_lengths(X, lengths):
            if lengths is None:
                yield 0, len(X)
                return
            end = np.cumsum(lengths).astype(int)
    ...
    def converged(self):
        ...
        return (self.iter == self.n_iter or
                (len(self.history) == 2 and
                 abs(self.history[1] - self.history[0]) < self.tol) or
                 self.iter - self.best_iter >= 10)

    def converged_gau(self):
        """``True`` if the EM algorithm converged and ``False`` otherwise."""
        # XXX we might want to check that ``logprob`` is non-decreasing.
        return (self.iter == self.n_iter or
                (len(self.history) == 2 and
                 abs(self.history[1] - self.history[0]) < self.tol * 100))
...
class _BaseHMM(BaseEstimator):
    ...
    """
    def __init__(self, inimodel, n_components=1,
                 startprob_prior=1.0, transmat_prior=1.0,
                 algorithm="viterbi", random_state=None,
                 n_iter=10, tol=1, verbose=False,
                 params=string.ascii_letters, 
                 init_params=string.ascii_letters,
                 lam = 0.8,shrink = 0.8,maxlam = 0.99, relax = "all"):
    ...
    def score_samples(self, O, F, lengths=None):
        ...
        for i, j in iter_from_X_lengths(O, lengths):
            framelogprob = self._compute_log_likelihood(self, O[i:j], F[i:j])
    ...
    def score(self, O, F, lengths=None):
        ...
        logprob = 0
        for i, j in iter_from_X_lengths(O, lengths):
            framelogprob = self._compute_log_likelihood(O[i:j,:], F[i:j,:])
            logprobij, _fwdlattice = self._do_forward_pass(framelogprob)
            logprob += logprobij
        return logprob

    def _decode_viterbi(self, O, F):
        framelogprob = self._compute_log_likelihood(O, F)
        return self._do_viterbi_pass(framelogprob)

    def _decode_map(self, O, F):
        _, posteriors = self.score_samples(O, F)
        logprob = np.max(posteriors, axis=1).sum()
        state_sequence = np.argmax(posteriors, axis=1)
        return logprob, state_sequence

    def decode(self, O, F, lengths=None, algorithm=None):
        ...
        for i, j in iter_from_X_lengths(O, lengths):
            ...
            logprobij, state_sequenceij = decoder(O[i:j,:], F[i:j,:])
            logprob += logprobij
            state_sequence[i:j] = state_sequenceij

        return logprob, state_sequence

    def predict(self, O, F, lengths=None):
        """Find most likely state sequence corresponding to ``X``.
        ...
        """
        _, state_sequence = self.decode(O, F, lengths)
        return state_sequence

    def predict_proba(self, O, F, lengths=None):
        ...
        """
        _, posteriors = self.score_samples(O, F, lengths)
        return posteriors


    def fit(self, O, F, lengths=None):
        ...
        self.fit_relax_all(O, F, lengths)
            
        return self 
  
    def fit_relax_all(self, O, F, lengths = None):
        self.tree = self.inimodel
        self.tree.fit(F,O.ravel())
        
        X = O - self.tree.predict(F).reshape(-1, 1)
    
        self.X = check_array(X)
        
        self._init(O, F, lengths=lengths)
        self._check()
        ...
        for iter in range(self.n_iter):
            ...
            for i, j in iter_from_X_lengths(X, lengths):				
                ...
                self._accumulate_sufficient_statistics(
                    stats, O[i:j], F[i:j], framelogprob, posteriors, fwdlattice,
                    bwdlattice)     
                #print(8)
                weights[i:j,:] = posteriors
#            print(framelogprob)
#            print(bwdlattice)
#            print(fwdlattice)
#            print(posteriors)
            ...
            #print(9)
            self._update_model(stats, O, F, weights)

            #print(10)
            self._do_mstep(stats, O, F, weights)
            
            #print(11)
            self._updateGM(stats, O, F, weights)
            

            self.monitor_.report(curr_logprob, self)
    ...
    def _update_model(self, stats, O, F, posteriors):
        '''
        update the model for fitting
        '''
        for i in range(self.n_components):
            #print(i)
            #print(sum(posteriors[:,i]))
            #self.lastmodel[i] = copy.deepcopy(self.model[i])
            if (sum(posteriors[:,i])>0.01):
                self.model[i].fit(F, O.ravel(),  posteriors[:,i])
    ...
    def _compute_posteriors(self, fwdlattice, bwdlattice):
        ...
        with np.errstate(under="ignore"):
            return np.nan_to_num(np.exp(log_gamma), posinf=1e10, neginf=-1e10)

    def _init(self, O, F, lengths):
