...
try:
    ...
    from core.base_DT import _BaseHMM
...
try:
    ...
    def fill_covars(covars, covariance_type, n_components, n_features):
        covars = np.asarray(covars)
        if covariance_type == "full":
            return covars
        if covariance_type == "diag":
            return np.array([np.diag(cv) for cv in covars])
        if covariance_type == "spherical":
            diag = np.tile(covars.reshape(-1, 1), (1, n_features))
            return np.array([np.diag(cv) for cv in diag])
        if covariance_type == "tied":
            return np.tile(covars[np.newaxis, :, :], (n_components, 1, 1))
        raise ValueError("Unsupported covariance_type: {}".format(covariance_type))
